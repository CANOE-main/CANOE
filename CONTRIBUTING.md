...
to submit code changes. It replaces the CONTRIBUTING files previously scattered across CANOE's
individual repos — this is the single source going forward.

## Reporting issues

Issues (bugs, feature requests, and questions) are tracked on [GitHub
Issues](../../issues) and are open to both internal team members and external users.
...
- `main` (or `stable`) holds the latest, battle-tested code. **Direct pushes to `main` are
  forbidden**, and this rule is enforced by the repo — please don't work around it.
...
- Squashing on merge is optional — use your judgment based on how many commits are worth
  preserving individually.
...
- Make sure the **DCO (Developer Certificate of Origin) check** passes. This can break after a
  rebase, merge, or when committing with certain tools (e.g. Copilot) — if your DCO check fails,
  this is usually why.
...
with the `question` tag or reach out to the dev team — see [Contact](contact.md).
